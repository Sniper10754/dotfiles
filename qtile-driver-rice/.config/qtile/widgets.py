...
PULSEAUDIO_WIDGET = widget.PulseVolume(fmt="VOLUME: {}", background=appearance.yellow)
...
SEP = widget.Sep(
    foreground=appearance.foreground, padding=7, linewidth=2, size_percent=100
)

PRIMARY_SCREEN_WIDGETS = [
    widget.CurrentLayout(background=appearance.purple),
    widget.GroupBox(
        block_highlight_text_color=appearance.yellow, highlight_method="block"
    ),
    widget.Spacer(),
    widget.WindowName(),
    widget.Spacer(),
